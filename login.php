...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($stmt) {
        ...
        if ($user) {
            ...
            $insert_stmt = $mysqli->prepare("INSERT INTO users (email, password, role) VALUES (?, ?, ?)");
            if ($insert_stmt) {
                $insert_stmt->bind_param("sss", $email, $hashedPassword, $role);
                if ($insert_stmt->execute()) {
                    ...
                    // Set default values for additional fields (if any)
                    $_SESSION['name'] = '';
                    $_SESSION['student_id'] = '';
                    $_SESSION['faculty'] = '';
...
<body class="bg-gray-100 flex items-center justify-center h-screen">
    <div class="bg-white p-8 rounded shadow-md w-full max-w-md">
        ...
        <form method="post" action="">
            ...
            <div class="mb-4">
                <select name="role" class="w-full p-2 border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-blue-500">
